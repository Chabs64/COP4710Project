
export default function Home() {
    return(
        <>
            <h2> Home Page </h2>
            <h3> Directory </h3>
            <p> For: PortEntry use path = /PortEntry  </p>
            <p> For: PortExit use path = /PortExit  </p>
            <p> For: PortAdmin use path = /PortAdmin  </p>
            <p> For: ShipRegistration use path = /ShipRegistration   </p>
            <p> For: TruckDriverRegistration use path = /TruckDriverRegistration   </p>
            <p> For: TruckRegistration use path = /TruckRegistration   </p>
            <p> For: CraneOperator use path = /CraneOperator  </p>
        </>
    )
}